...
// `ua: en` 时用 title_en / options_en / correctAnswer_en 覆盖对应字段。
...
onRecordEnrich((e) => {
  try {
    ...
    if (lang === "en") {
      const titleEn = e.record.get("title_en");
      if (titleEn) {
        e.record.set("title", titleEn);
      }

      const optionsEn = e.record.get("options_en");
      if (optionsEn !== null && optionsEn !== undefined && optionsEn !== "") {
        e.record.set("options", optionsEn);
      }

      const correctEn = e.record.get("correctAnswer_en");
  ...
  // 永远不向客户端暴露 *_en 辅助字段
  e.record.hide("title_en", "options_en", "correctAnswer_en");
